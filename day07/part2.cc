...
const uint64_t DISK_SIZE       = 70000000;
const uint64_t MIN_UNUSED_SIZE = 30000000;
...
int main()
{
  ...
  uint64_t unused_size = DISK_SIZE - dir_size_map["/"];
  uint64_t goal_size = MIN_UNUSED_SIZE - unused_size;
  uint64_t result = MIN_UNUSED_SIZE;

  for (auto it = dir_size_map.begin(); it != dir_size_map.end(); it++) {
    if (it->second >= goal_size) {
